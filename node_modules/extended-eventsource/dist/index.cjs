...
class CustomEventSource extends EventTarget {
  ...
  constructor(url, initDict) {
    ...
    if (this.options.omitCredentials && this.options.withCredentials) {
      this.logger?.warn('omitCredentials and withCredentials have been set to true. withCredentials will be ignored and credentials will not be sent!');
    }
    this.connect();
  }
  ...
  async connect(lastEventId) {
    ...
    const fetchOptions = {
      ...
      cache: 'no-store',
      credentials: this.options.omitCredentials ? 'omit' : this.withCredentials ? 'include' : 'same-origin',
    ...
    try {
      ...
      const response = this.options.fetch ? await this.options.fetch(request) : await globalThis.fetch(request);

      // https://html.spec.whatwg.org/multipage/server-sent-events.html#dom-eventsource (Step 15)
      if (response.status !== 200) {
        return this.failConnection(`Request failed with status code ${response.status}`, request, response);
      } else if (!response.headers.get('Content-Type')?.includes(ContentTypeEventStream)) {
        return this.failConnection(`Request failed with wrong content type '${response.headers.get('Content-Type')}'`, request, response);
      } else if (!response?.body) {
        return this.failConnection(`Request failed with empty response body'`, request, response);
      }
      ...
      await this.reconnect('Reconnecting EventSource because of error', error, request);
      return;
    }
  ...
  async reconnect(msg, error, request) {
  ...
  failConnection(error, request, response) {
  ...
  announceConnection(request, response) {
